      ...
      <div class="drap-node-content" v-if="chooseItem">
        <div>
          <div
            ...
            :style="{'background-image':'url('+emptyNode.avatarUrl+')'}"
          ></div>
        ...
        <div v-for="(item,idx) in data" :key="idx">
          <div
            ...
            :style="{'background-image':'url('+item.avatarUrl+')'}"></div>
          <h4>{{item.name}}</h4>
        </div>
      </div>
    </ul>
...
export default {
  ...
  data() {
    return {
      ...
      emptyNode: {
        ...
        appellation: "未知",
        status: 1,
        avatarUrl: require("../../../static/images/node_null.png")
      }
    };
  },
  beforeMount() {
  ...
  methods: {
    ...
    resolvePath (_path) {
      let result = Util.resolveFilePath(_path)
      return result
    }
  }
};
