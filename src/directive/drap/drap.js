...
function updateDrapEle(e) {
  ...
  // 边界处理（记录下结束的位置）
  const _M_ELE_left = endX = eleLeft < 0 ? 0 : (eleLeft > (W_width - drapEleSize) ? (W_width - drapEleSize) : eleLeft)
  const _M_ELE_top = endY = eleTop < 0 ? 0 : (eleTop > (W_heiht - drapEleSize) ? (W_heiht - drapEleSize) : eleTop)
...
function endDrapEle(e, cb) {
  ...
  let point = {x:endX+drapEleSize/2,y:endY+drapEleSize/2}
  document.body.removeChild(nodesEle);
  if ((startX > endX + 88) || (startY > endY + 88)) {
    cb ? cb(point) : ''
  }
}