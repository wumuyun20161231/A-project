...
// 简化选择器
function _selector2id (selector, sex) {
  var result = []
  var hash = {}
  if (sex < 0) {
    // 如果自己的性别不确定
...
function _getAge (_customKey, _data) {
  let result = 0
  _data.forEach(item => {
    if (item.customKey === _customKey) {
      result = item.age
    }
    Array.isArray(item.linkNodes) && item.linkNodes.forEach(sitem => {
      if (sitem.customKey === _customKey) {
        result = sitem.age
      }
    })
  })
...
// 根据年龄判断是否是姐妹
function _resolveByAge (_customKey, _ids, _currentItem, _data) {
  // console.log(`_resolveByAge`, _customKey, _ids, _currentItem, _data)
  let customKey = _customKey.split(',')
  let ids = _ids[0] ? _ids[0].split(',') : _ids[1].split(',')
  let idsS = ids.join(',')
  customKey.unshift('')
  const CLENGTH = customKey.length
  const ILENGTH = _ids.length

  let currentAge = _currentItem.age

  let targetAge = 0

  // if (+_currentItem.hierarchy === 0 && customKey[CLENGTH-1] !== "h" && customKey[CLENGTH-1] !== "w") {
  //   targetAge = _getAge(null, _data)
  // }else if (+_currentItem.hierarchy === 0 && (customKey[CLENGTH-1] === "h" || customKey[CLENGTH-1] === "w")) {
  //   let _c = []
  //   _ids[0].split(',').forEach((item, index)=> {
  //     if (index < _currentItem.hierarchy) {
  //       _c.push(item)
  //     }
  //   })
  //   targetAge = _getAge(_c.join(','), _data)
  // } else if (+_currentItem.hierarchy > 0) {
  //   let _c = []
  //   _ids[0].split(',').forEach((item, index)=> {
  //     if (index < _currentItem.hierarchy) {
  //       _c.push(item)
  ...
  //   targetAge = _getAge(_c.join(','), _data)
  // }
  // console.log(`_resolveByAge`, customKey, _ids, customKey[1] === "f" || customKey[1] === "m", customKey[CLENGTH-1] === "s" || customKey[CLENGTH-1] === "d")
  let result = []
  if (+_currentItem.hierarchy === 0) {
    targetAge = _getAge(null, _data)
    if (ids.length <= 2) {
      if (idsS.indexOf('xb') > -1) {
        result[0] = idsS.replace(/xb/, (+currentAge > +targetAge) ? 'ob' : 'lb')
      } else if (idsS.indexOf('xs') > -1) {
        result[0] = idsS.replace(/xs/, (+currentAge > +targetAge) ? 'os' : 'ls')
      } else {
        result[0] = idsS
      }
    } else if (ids.length >= 3 && ids[ids.length-1] === 's') {      
      ids[ids.length-1] = (+currentAge > +targetAge) ? 's&o' : 's&l'
      // console.log(`s&ol`,ids, (+currentAge > +targetAge) ? 's&o' : 's&l')
      result[0] = ids.join(',')
    } else if (ids.length >= 3 && ids[ids.length -1] === 'd') {      
      ids[ids.length-1] = (+currentAge > +targetAge) ? 'd&o' : 'd&l'
      // console.log(`d&ol`, ids, (+currentAge > +targetAge) ? 'd&o' : 'd&l')
      result[0] = ids.join(',')
    } else if (ids.length >= 3 && ids[ids.length-1] === 'w') {      
      ids[ids.length-2] = (+currentAge > +targetAge) ? 's&o' : 's&l'
      // console.log(`s&ol`,ids, (+currentAge > +targetAge) ? 's&o' : 's&l')
      result[0] = ids.join(',')
    } else if (ids.length >= 3 && ids[ids.length - 1] === 'f') {
      // 亲家公
      if (ids[0] === 's' && ids[1] === 'w' && ids[2] === 'f' && ids.length === 3) {

      } else {
        ids[ids.length - 2] = (+currentAge > +targetAge) ? 'd&o' : 'd&l'
      }
      // console.log(`d&ol`, ids, (+currentAge > +targetAge) ? 'd&o' : 'd&l')
      result[0] = ids.join(',')
    } else {
      if (ids[0].indexOf('xb') > -1) {
        result[0] = idsS.replace(/xb/, (+currentAge > +targetAge) ? 'ob' : 'lb')
      } else if (idsS.indexOf('xs') > -1) {
        result[0] = idsS.replace(/xs/, (+currentAge > +targetAge) ? 'os' : 'ls')
      } else {
    ...
    // console.log(`_currentItem.hierarchy`, ids)
    targetAge = _getAge(_c.join(','), _data)
    if (ids[0].indexOf('xb') > -1) {
      result[0] = idsS.replace(/xb/, (+currentAge > +targetAge) ? 'ob' : 'lb')
    } else if (idsS.indexOf('xs') > -1) {
  ...
  // if (CLENGTH > 2 && (customKey[1] === "f" || customKey[1] === "m") &&  (customKey[CLENGTH-1] === "s" || customKey[CLENGTH-1] === "d" || customKey[CLENGTH-1] === "h" || customKey[CLENGTH-1] === "w")) {
  //   if (_ids[0].indexOf('xb') > -1) {
  //     result[0] = _ids[0].replace(/xb/, (+currentAge > +targetAge) ? 'ob' : 'lb')
  //   } else if (_ids[0].indexOf('xs') > -1) {
  //     result[0] = _ids[0].replace(/xs/, (+currentAge > +targetAge) ? 'os' : 'ls')
  //   }
  //   console.log(`判断关系`, result)
  // } else {
  //   result[0] = _ids[0]
  // }
  // console.log(`_resolveByAge-result`, result)
  return result
}

export const getCallNameByCustomKeyForMe = (customKey, item, data, centerNode) => {
  let _options = {
    text: '',
    customKey: customKey,
    sex: -1,
  ...
  if (customKey) {
    let ids = _resolveByAge(customKey, _selector2id(',' + customKey, _options.sex), item, data)
    for (var j = 0; j < ids.length; j++) {
      var id = ids[j]      
      var items = _getDataById(id)
      if (items.length) {
        result = result.concat(items)
      } else if (id.indexOf('w') == 0 || id.indexOf('h') == 0) {
        // 找不到关系，随爱人叫
        items = _getDataById(id.substr(2))
        if (items.length) {
          result = result.concat(items)
        }
      }
    }
    result = _unique(result)[0]
  } else {
    result = '自己'
  }
